...
SAM_CHECKPOINT = r"c:\Users\user\Downloads\segment-anything-main\segment-anything-main\weights\sam_vit_b_01ec64.pth"
...
def get_orientation_from_mask(mask_roi):
    """
    由二值 mask (roi 區域) 計算主要方向，回傳角度 (0-360)
    mask_roi: 二值或 0/1 uint8 的區域 (shape: h x w)
    """
    try:
        if mask_roi is None:
            return 0.0
        ...
        if len(idx_pos[0]) > 0:
            width_pos = np.max(cross_dists[idx_pos]) - np.min(cross_dists[idx_pos])
        if len(idx_neg[0]) > 0:
            width_neg = np.max(cross_dists[idx_neg]) - np.min(cross_dists[idx_neg])
...
def calculate_flange_target(spoon_x, spoon_y, target_angle_deg):
    ...
    return cmd_x, cmd_y
# ========= 4. 核心流程 =========
def scan_workspace_for_all_items(pipeline, align, model, K, depth_scale, sam_predictor=None):
    print(f"--- 開始 3D 掃描 ---", flush=True)
    ...
    try:
        ...
        while time.time() - start_time < SCAN_DURATION_SECONDS:
            loop_i += 1
            print(f"[scan] loop {loop_i}, elapsed={time.time()-start_time:.2f}s", flush=True)
            try:
                frames = pipeline.wait_for_frames(timeout_ms=5000)
            except Exception as e:
                print(f"[scan] wait_for_frames exception: {e}", flush=True)
                continue
            aligned = align.process(frames)
            color_frame = aligned.get_color_frame()
            depth_frame = aligned.get_depth_frame()
            if not color_frame:
                print("[scan] no color_frame, continue", flush=True)
                continue
            
            try:
                depth_map_m = np.asanyarray(depth_frame.get_data()) * depth_scale
            except Exception as e:
                print(f"[scan] depth_frame->array exception: {e}", flush=True)
                depth_map_m = np.zeros((RS_HEIGHT, RS_WIDTH))
            display_frame = np.asanyarray(color_frame.get_data())
            display_frame = cv2.rotate(display_frame, cv2.ROTATE_180)

            # 若有 SAM predictor，設定當前 RGB 影像
            ...
            try:
                ...
                print(f"[scan] model.track exception: {e}", flush=True)
                result = None
            
            last_annotated_frame = display_frame.copy() 

            if result is None or result.boxes.id is None:
                print("[scan] no tracking boxes", flush=True)
                cv2.imshow(WIN_NAME, display_frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
                continue

            print(f"[scan] boxes count: {len(result.boxes)}", flush=True)
            for box in result.boxes:
                ...
                if safe_x2 > safe_x1 and safe_y2 > safe_y1:
                    detected_angle = 0.0
                    # 若有 SAM predictor，使用 SAM mask 來計算方向
                    if sam_predictor is not None:
                        try:
                            # 使用原始 box（float）作為 SAM prompt
                            ...
                            mask_roi = mask[safe_y1:safe_y2, safe_x1:safe_x2]
                            detected_angle = get_orientation_from_mask(mask_roi)

                            # 畫出 SAM 輪廓 (offset 回原影像座標)
                            contours, _ = cv2.findContours((mask * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                            for cnt in contours:
                                cnt_shift = cnt + np.array([[0, 0]])
                                cv2.drawContours(display_frame, [cnt_shift], -1, (0,255,0), 2)
                        except Exception:
                            detected_angle = 0.0
